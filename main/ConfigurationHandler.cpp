...
void ConfigurationHandler::registerClass(
		SavingInterfaceClass *registerclassptr) {
...
void ConfigurationHandler::LoadAllConfiguration() {
	...
	std::for_each(_ll.begin(), _ll.end(),
			[&](SavingInterfaceClass *n) {
				cJSON *objectJSONClass = cJSON_GetObjectItem(root,
						n->GetClassFriendlyName().c_str());
